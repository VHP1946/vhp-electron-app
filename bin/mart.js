...
class LocalMart{
    ...
    constructor({
      ...
      apiargs={}
        ...
        try{fs.mkdirSync(this.folder);}catch{}//ensure folder exist
        ...
        this.local=new NEDBconnect(path.join(this.folder,this.file),locstore.ensure);
        this.changes=new NEDBconnect(path.join(this.root,'/changeslogs/',this.file),locstore.ensure);
    ...
    INSERTdoc=(idoc)=>{
      return new Promise((resolve,reject)=>{
        if(idoc){
          this.local.INSERTdb(idoc).then(({err,doc})=>{
